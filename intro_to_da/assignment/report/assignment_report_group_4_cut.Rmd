...
Cosine similarity is just one method for developing a flexibility score. An alternative is to use the _Jaccard similarity_ given by \eqref{eq:jaccard}.

\begin{equation} \label{eq:jaccard}
J(A, B) = \dfrac{|A\cap B|}{|A \cup B|}
\end{equation}

In \eqref{eq:jaccard} it is seen that the similarity of two sets can be compared by comparing the relative differences in their [intersection](https://en.wikipedia.org/wiki/Intersection_%28set_theory%29) and [union](https://en.wikipedia.org/wiki/Union_%28set_theory%29). 
...
Treating each individuals' picks on each network as a different set of picks, the Jaccard distance was calculated for each possible combination (e.g. popularity with design, implementation, and advocacy; design with implementation and advocacy etc).
...
This was then combined with the Jaccard distance scores and an overall flexibility score derived by multiplying the mean Jaccard distance for each ID's picks by the mean "picks proportion score" derived above. In this way, ID's were penalised for choosing fewer picks in each network. 

This was done as, for two IDs with the same mean Jaccard distance, a higher flexiblity score should be assigned to the ID that made 6 picks in each network than the ID that made 1 pick in each network.
...
Having developed this new, alternative score the previous flexiblity score calculated using cosine similarity was compared. The results are presented below.
...
flex_bonus %>% 
  na.omit %>% 
  kable(caption = "Cosine and Jaccard flexibility scores and Z-values",
...
TODO: explain differences in score - degeneracy in cosine sim vs. nature of jaccard giving different scores
