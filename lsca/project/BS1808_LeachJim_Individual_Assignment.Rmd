...
There are two methods that can be applied when diagnosing the two modelling approaches. The first approach considers the models in isolation, and looks at in-sample measures of fit to estimate model quality. The second approach is to forecast using each model and compare the results with the _test_ data, summarising differences using common error metrics. Both approaches have been taken and are discussed below.
...
train <- train %>% mutate(hw_resid_plot = hw_resid %>% map(plot_hw_resid))
```

In order to assess if the residuals from each model had constant variance, ACF plots were used, displaying the first 40 lags.

```{r acf_plots}
plot_acf <- function(residuals, lag = 35) {
  vals <- acf(residuals, lag.max = lag, plot = F)
  
  title <- paste0("ACF plot of residuals up to lag ", lag, " (", lag/7, " weeks)")
...
The average residual was `r round(mean(train$hw_resid[[1]]), 2)`. The plot of the residuals and the ACF plot are displayed in figures below.

```{r hw_resid1, fig.align='center'}
train$hw_resid_plot[[1]]
```

```{r hw_acf1, fig.align='center'}
train$hw_acf[[1]]
```

The results from the Ljung-Box test are presented in table TODO.

```{r hw_lb1}
kable(train$hw_lb[[1]],
      col.names = lb_names,
      caption = "Table TODO: Ljung-Box test statitics",
      digits = 3)
...
The average residual was `r round(mean(train$hw_resid[[2]]), 2)`. The plot of the residuals and the ACF plot are displayed in figures below.

```{r hw_resid2, fig.align='center'}
train$hw_resid_plot[[2]]
```

```{r hw_acf2, fig.align='center'}
train$hw_acf[[2]]
```
...
The average residual was `r round(mean(train$hw_resid[[3]]), 2)`. The plot of the residuals and the ACF plot are displayed in figures below.

```{r hw_resid3, fig.align='center'}
train$hw_resid_plot[[3]]
```

```{r hw_acf3, fig.align='center'}
train$hw_acf[[3]]
```
...
The average residual was `r round(mean(train$hw_resid[[4]]), 2)`. The plot of the residuals and the ACF plot are displayed in figures below.

```{r hw_resid4, fig.align='center'}
train$hw_resid_plot[[4]]
```

```{r hw_acf4, fig.align='center'}
train$hw_acf[[4]]
```
...
forcast_fortnight <- function(ts_model) {
  forecast(ts_model, h = 14)$mean %>% as.numeric()
}

# Forecast for 14 days
train <- train %>% 
          mutate(hw_forecast = hw %>% map(forcast_fortnight),
                 arima_forecast = arima %>% map(forcast_fortnight))
```
...
kable(mad, 
      ...
      digits = 1,
      caption = "Table TODO: MAD Values for ARIMA and Holt-Winters forecasted values when compared to those in the the testing set.")
```
  
These results are presented in table TODO. Although there are differences between the two methods at a store level; overall it is the case that ARIMA performs better than Holt-Winters, with an average MAD value of `r scales::comma(round(mean(mad$arima), 1))`, compared to `r scales::comma(round(mean(mad$hw), 1))` for Holt-Winters. 