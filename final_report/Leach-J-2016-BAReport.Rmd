...
This document is broken down in the several sections. The introduction presents some background, discusses the motivations for and analytical approaches to this work, and provides a brief summary of some relevant literature. The methodology section describes the analytical steps, and provides the `R` code used in the analysis to make this work reproducible (the necessary data are also provided). The results section contains summary statistics and visualisations for the analysis, and the conclusion provides some final remarks and recommendations. Some additional tables and visualisations are presented in the appendices and a full list of references is provided. 
...
In this analysis, a novel approach to designing fare systems has been investigated. Graph-analysis methods have been used to model the TfL Underground network as a graph with stations as _nodes_ [@NewmanBook:2010] and links between adjacent stations as _edges_ [@NewmanBook:2010]. 

Specifically, the creation of new fare zones based on the graph properties of the network have been adopted and evaluated. This was done to understand if data-driven methods could be used as one of the "innovative strategies" TfL will require in order to meet its financial reqirements. Re-zoning of the TfL tube could provide the basis for changing pricing in order to increase fare revenue.

Two main approaches were performed:
...
In each case the resulting groups were used to define new, alveolar fare zones [@montella_multimodal_2014]. These zones were then compared with the existing fare zones using a variety of methods.
...
For zones defined through community-detection algorithms, graph theoretical measures proposed by @yang_defining_2015 were also used to evaluate the new and existing zones. Finally, the zones generated via these alternative approaches were rated against criteria set out by @daskin_quadratic_1988 for measuring the suitability of pricing systems.
...
Daskin et. al. developed a model to test several alternative pricing models, and found that zone-based pricing was most optimal when price-sensitivty was not strongly related to trip distance. 
...
* __Munipal zones__ based on existing political boundaries were also easy for customers to understand, but resulted in some short journeys crossing many zones (and therefore being expensive), and long journeys remaining in just one zone (and therefore being cheap). This countered existing tranport pricing theory, and so was seen as a negative result.
...
Pricing of public transport journies has been seen to generally be dependent on the distance travelled. Zone-based approaches extends this to include measures such as origin/destination importance into the price. 

The use of smart-card technology, whilst also providing many benefits, has enabled and encouraged increasingly sophisticated analyses of transport systems to be undertaken. This has included efforts to use graph theory for route optimisation, and the evaluation of the importance of stations in the network.

These factors show that approaching the design of fare-zones through graph-theoretical methods is not only novel, but also has basis in existing theory. Node importance measures are an alternative to arbitrary decision about which stations are important, and both these and community-detection methods can be used to capture existing practices for pricing journies based on distance travelled. The use of advanced data and analytical approaches for transport issues has also been shown to have academic justification.
...
Examples of these data can be seen in appendix one.
...
Metrics suggested by @yang_defining_2015 were used to measure the "goodness" of the detected communities for the community-based aproach. The same metrics were calculated for the existing fare zones and the results compared.
...
The importance-based zones did not appear to be geographically distinct from one another (appendix TODO) in the same way as the existing fare zones. However, when measured with Salton's [-@Salton:1986:IMI:576628] cosine measure, the zones defined using station importance metrics generally showed a high similarity with the existing fare zones (table `r table`). This was slightly surprising given the forced, equal distribution of the number of stations in each importance-based zone (not the case for the existing zones).
...
Zones created through closeness most closely matched the figures for the current zones, which was to be expected given the similarities in how they are defined. The increase in fares based on closeness zones relative to the current zone model was probably due to the equal distribution of stations to zones enforced by the decile-rank grouping approach creating a greater number of (more expensive) intra-zone journies. 
...
Whilst not as geographically distinct as the edge-betweenness communities, the spinglass algorithm generated generated clear communities which could be used to create fare zones relatively simply, especially with the use of other data, such as socio-ecomoic factors. 
...
Node importance measure-based zoning approaches showed good overlap with existing the zones, and could result in potentially higher fare revenues under the existing charge scheme. Community-detection algorithms were able to identify groups of stations with good graph-theoretical properties, and which showed a similar geographic distinction as the existing zones. 
...
#### Appendix `r app`: _Examples of raw data used_

__Raw graph structure data__:
