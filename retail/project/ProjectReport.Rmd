...
In order to understand the market structure, simple numerical summaries and several visualisations were created. Total and per-shop sales per week were charted, along with weekly market shares for each shop. The relationship between price and market share was plotted for each shop, as was the relationship between promotions and market share. 
...
Using the _wide_ format data, own and cross-price elasticities were calculated for each shop. Model selection for performing this calculation was done in a similar manner to that in earlier work (Leach et al, 2016), i.e. using cross-validation and mean-squared error to compare various models. 

Calculating elasticities at an individual consumer segment level lead to insignificant and/or strange results (e.g. negative cross-price elasticities), and so the calcualtions were performed at an overall level, i.e. accross all three customer segments. Clout and vulnerability statistics were also created and visualised. 
...
From the data present in figure TODO it is clear that Morrisons increases its market share at the expense of Sainsburys and Tesco primarily, and also from Aldi and Lidl ("discounters"). Note that figure TODO shows an overall level view for all customer segments. The pair-wise patterns remain constant accross the three individual customer segments analyses. However the numerical values of the correlations change slightly, and as such the values for each consumer segment are presented in appendix TODO.

Shop-level switching matrices were also computed accross the three customer segments. The overall matrix is presented in table TODO and the customer-segment level matrices in appendix TODO. These matrices show which shops customers are switching between and hence can be used to estimate competition between shops. For the overall matrix it can be seen that customers that shopped at Morrisons also shopped at Asda and Tesco, with some switching to Sainsburys as well. It is also possible to estimate just how much switching is occurring in the market place. At the overall level, around `r scales::percent(switch_all)` of visits were "switched" visits, i.e. those in which the customer was shopping at a different shop than on their previous visit. This shows that there is some consumer movement between shops within the market. It should be noted that this value is not constant across the three consumer segments, with light, medium and heavy users showing `r scales::percent(switch_l)`, `r scales::percent(switch_m)`, and `r scales::percent(switch_h)` of "switched" visits, respectively (based on switching matrices in appendix TODO). 

The repeat purchase rate for each shop can also be viewed for each consumer segment (figure TODO). It can be seen that Morrisons' actually has a relatively constant repeat purchase rate across the three consumer segments (around 60%), although it is slightly lower for the heavy segment. 
...
From the result in table TODO it is possible to see that, at a shop level, consumers are relatively price _inelastic_ (i.e. the absolute values of price elasticity are less than one), indicating that they are not particularly sensitive to the average price at each shop. This is in contrast with earlier work (Leach et al, 2016) which showed that at a _brand_ level, consumers are much more price sensitive. Even within this setting, however, it appears that Morrisons is at risk from a price perspective. It has very little clout, meaning that its price has no effect on sales at other shops, but is highly vulnerable, meaning that the price of other shops (Tesco and Asda in this instance)   can have a large effect on its sales. 

The lack of shop-level price sensitivity is likely due to limitations of the data: they cover purchases of coffee only. Consumers in reality are probably highly sensitive to the average cost of a trip to each of the supermarkets. However, looking only at coffee sales does not show this trend, and so such sensitivities are not observed. Overall shop competition could be better measured with richer data, covering entire basket purchases at each shop.
