...
Given that we seek to minimise there, the equality constraint can be relaxed to a greater than or equal to constraint. This will still resolve to an equality as if we pick _any_ theta that is greater than $|y_i - \beta_{years}*years - ... |$ then there is another value of theta that _will_ improve the solution, and so this still resolves to an equality. Thus:
