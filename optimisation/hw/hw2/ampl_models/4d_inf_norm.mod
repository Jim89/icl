...
# set the constraints
subject to c1 {i in NUM}: theta>= comp[i]-(beta_years*years[i] + beta_change_stock*change_stock[i] + beta_change_sales*change_sales[i] + beta_mba*mba[i]);
subject to c2 {i in NUM}: theta>= (beta_years*years[i] + beta_change_stock*change_stock[i] + beta_change_sales*change_sales[i] + beta_mba*mba[i]) - comp[i];