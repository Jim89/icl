...
This is less than the predicted birth weight when `cigs` is 0, implying that their is a negative correlation between smoking and birth weight (i.e. the more cigarettes are smoked during pregnancy, the lower the expected birth weight). 
...
This occurs due to the fact that the $\hat\beta_0$ value of `r round(b0_2, 2)` is lower than the predicted birth weight of 125. I.e. if no cigarettes are smoked, the estimated birth weight is `r round(b0_2, 2)` ounces. Given that the $\hat\beta_1$ value is also negative, it follows that a "negative value" of cigarettes is the only way to increase the predicted birth weight about the $\hat\beta_0$ value. In reality this is more likely to represent not smoking at all during pregnancy.

## 2e

If 85% of women in the sample do not smoke whilst pregnant then it would be expected that, given then ceteris paribus assumption, 85% of women would have an infant with birth weight of approximately `r round(b0_2, 2)` (as the second term in the SRF \eqref{eq:srf_2} will evaluate to 0).

```{r 2e}
prop <- mean(data$bwght >= b0_2)
```

Approximately `r paste0(100*round(prop, 2), "%")` of women in the sample have a child with a birth weight greater than or equal to the $\hat\beta_0$ value.

This is broadly consistent with the findings from section $2d$. However, the fact that this value differs from the expected value (of 85%) under the ceteris paribus assumption demonstrates that this assumption is likely not holding in this case, and that factors other than _just_ the mother's smoking habit are affecting the weight of the infant.
