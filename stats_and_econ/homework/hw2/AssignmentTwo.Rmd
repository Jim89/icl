...
fit1c_summary <- summary(fit1c)
fit1c_prpblck_coef <- fit1c_summary$coefficients[2, 1]
```

The new value for the $\hat\beta_{prpblck}$ coefficient is `r round(fit1c_prpblck_coef, 3)`, i.e. the expected percentage change in the median soda price given a one unit increase in the proprtion/percentage of black residents is now `r round(fit1c_prpblck_coef*100, 2)`%. This is lower than when the proportion of residents living in poverty is not controlled for, i.e. $\hat\beta_{prpblck}$ has decreased.
...
The correlation between $log(income)$ and $prppov$ is `r round(corr_income_pov, 3)`, i.e. there is a negative correlation between the two and it is strong. From intution, this is roughly expected, i.e. as the median income in a ZIP-code increases, it would be expected that the proportion of residents living in poverty would decrease.

## 1e

> "Because $log(income)$ and $prppov$ are so highly correlated, they have no business being in the same regression."

This is an issue of __(multi)collinearity__, a common topic in regression. [Consequences](https://en.wikipedia.org/wiki/Multicollinearity#Consequences_of_multicollinearity) of multicollinearity include potential imprecisions in the parameter estimates for independant variables, redundancy of information in the model (which can lead to overfitting), and larger standard errors in affected coefficients (which may lead to a failure to reject the null hypothesis). (Wikipedia: 2015)
...
However, in general it can be unwise to include highly correlated variables in a model due to the consequences that can arise if this is does. Additionally, inclusion of an irrelvant variable will lead to larger variances in the $\hat\beta_j$ parameter etstimates. As such, if two variables, $x_1$ and $x_2$ are both included in a model but are highly correlated, then it can be seen that either of these variables is irrelevant, given the presence of the other. Therefore the variance of the other $\hat\beta_j$ parameters will increase, and so it is unwise to include both in the model.
...
In terms of the model parameters, the null hypothesis that, after controlling for $sales$ and $roe$, $ros$ has no effect on CEO salary is given by \eqref{eq:2anull}, and alternative is given by \eqref{eq:2aalt}.
...
The $t$-value can be calculated, or extraced from the linear model.
